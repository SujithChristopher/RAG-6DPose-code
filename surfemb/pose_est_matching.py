...
def estimate_pose(mask_lgts: torch.tensor, query_img: torch.tensor,
    ...
    with timer('corr matrix', debug):
        ...
        # print(corr_matrix.shape) # torch.Size([5476, 73132])
        if max_pool:
            # max pool spatially
    ...
    if poses is None:
        with timer('sample corr', debug):
            ...
            # print(p2d_idx.shape,p3d_idx.shape) # torch.Size([10000, 4]) torch.Size([10000, 4])
            # print(p2d_idx) #tensor([[3378, 2565, 4391, 2558],
            #                         #[3065, 2491,  914, 1905],
            # print(p2d.shape,p3d.shape) # torch.Size([10000, 4, 2]) torch.Size([10000, 4, 3])
            # print(p2d) # tensor([[[48., 45.],
